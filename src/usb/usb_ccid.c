 ...
 */
static void build_response_header(uint8_t *response, uint8_t msg_type,
                                   uint32_t length, uint8_t slot, uint8_t seq,
 ...
 */
static int handle_icc_power_on(const uint8_t *data, size_t len,
                                uint8_t *response, size_t *resp_len)
{
    ...
    build_response_header(response, CCID_RDR_TO_PC_DATABLOCK,
                          ccid_state.atr_len, hdr->bSlot, hdr->bSeq,
 ...
 */
static int handle_icc_power_off(const uint8_t *data, size_t len,
                                 uint8_t *response, size_t *resp_len)
{
    const ccid_header_t *hdr = (const ccid_header_t *) data;
    ...
    build_response_header(response, CCID_RDR_TO_PC_SLOTSTATUS,
                          0, hdr->bSlot, hdr->bSeq,
                          CCID_CMD_STATUS_OK, 0);
 ...
 */
static int handle_get_slot_status(const uint8_t *data, size_t len,
                                   uint8_t *response, size_t *resp_len)
{
    const ccid_header_t *hdr = (const ccid_header_t *) data;
...
static int process_apdu(const apdu_command_t *cmd, apdu_response_t *resp)
{
    ...
    if (ccid_state.selected_app < MAX_APPS &&
        app_registry[ccid_state.selected_app].active) {
 ...
 */
static int handle_xfr_block(const uint8_t *data, size_t len,
                             uint8_t *response, size_t *resp_len)
{
    const ccid_header_t *hdr = (const ccid_header_t *) data;
    apdu_command_t cmd;
    ...
    LOG_DEBUG("CCID: APDU CLA=%02X INS=%02X P1=%02X P2=%02X Lc=%d",
    ...
    build_response_header(response, CCID_RDR_TO_PC_DATABLOCK,
                          response_len, hdr->bSlot, hdr->bSeq,
                          CCID_CMD_STATUS_OK, 0);
...
int usb_ccid_process_message(const uint8_t *data, size_t len,
                              uint8_t *response, size_t *resp_len)
{
    ...
    LOG_DEBUG("CCID: Message type 0x%02X, slot %d, seq %d, len %u",
              hdr->bMessageType, hdr->bSlot, hdr->bSeq, hdr->dwLength);
