...
uint8_t ctap2_process_request(const ctap2_request_t *request, ctap2_response_t *response)
{
    ...
    switch (request->cmd) {
        case CTAP2_CMD_MAKE_CREDENTIAL:
            return ctap2_make_credential(request->data, request->data_len,
                                        response->data, &response->data_len);

        case CTAP2_CMD_GET_ASSERTION:
            return ctap2_get_assertion(request->data, request->data_len,
                                      response->data, &response->data_len);
        ...
        case CTAP2_CMD_CLIENT_PIN:
            return ctap2_client_pin(request->data, request->data_len,
                                   response->data, &response->data_len);
